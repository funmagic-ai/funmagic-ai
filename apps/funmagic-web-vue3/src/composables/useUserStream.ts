/**
 * Singleton SSE connection manager for the web app.
 *
 * Manages one SSE connection per browser tab to `/api/stream`.
 * All task progress events are multiplexed through this single connection.
 * Subscribers register by taskId; events are routed by the `taskId` field.
 */
      ...
      while (true) {
        ...
        for (const block of blocks) {
          ...
          try {
            ...
            // Route event to subscriber by taskId
            const id = (parsed.taskId as string) || (parsed.messageId as string)
            if (id) {
...
/**
 * Subscribe to events for a specific task ID.
 * Lazily connects to the SSE stream on first subscriber.
 * Returns an unsubscribe function.
 */
export function subscribeToTask(id: string, callback: EventCallback): () => void {
